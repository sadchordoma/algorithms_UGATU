...
int main() 
{
    ...
    {    // Обходим по индексам строки stroka1
        Stack_of_letters.push(stroka1[index]);  // Добавляем в Стек букв одну букву из строки stroka1
        if ( Stack_of_letters.top() == stroka2[index_stroki2] ) 
        {     // Если верхушка Стека равна букве второй строки, то заходим в цикл while
            while ( !Stack_of_letters.empty() ) 
            {     // и проверяем пока стек не пуст, совпадает ли вершина стека с буквой второй строки
                if ( Stack_of_letters.top() == stroka2[index_stroki2] ) 
                { // Если это так, то убираем этот элемент из стека, увеличиваем индекс строки2 на 1 и проверяем следующие буквы в стеке
                    Stack_of_letters.pop();
                    index_stroki2++;
                } else 
                {            // Если верхушка стека не равна букве второй строки, то прерываем цикл
                    break;
                }
            }
        }
        if ( index_stroki2 == size && Stack_of_letters.empty() ) 
        {    // Проверяем, если индекс 2-й строки равен длине строк и Стек букв пуст, то значит все буквы совпали и мы очистили наш стек
